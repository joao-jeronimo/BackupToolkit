...
def do_rsync(be, portnr, orig_username, orig_ip, orig_path, dest_path):
    rsync_arguments = " ".join(
              [ "-rltv", "--delete" ]
            + [ "--exclude=\"%s\""%toexclude for toexclude in be.config.BackupToolkit.RSYNC_EXCLUSIONS ]
        )
    ...
    rsync_command = ('%(rsync_bin)s %(rsync_arguments)s --port="%(portnr)d" "%(orig_username)s@%(orig_ip)s:%(orig_path)s" "%(dest_path)s"' % {
                    'rsync_bin':        be.config.BackupToolkit.RSYNC_BIN,

...
def check_dataset_existence(c):
    missing_datasets = [
        misspair
        for misspair in [   ( miss['localspecs']['dest_path'], bkhelpers.path_to_datasetname(miss['localspecs']['dest_path']))
                for miss in bkconfigs.BACKUP_PROFILES.values()   ]
...
def check_fix_zfs_mounts(c, force_mount_datasets=False):
    ...
    full_zfs_list = ( [bkconfigs.ZFS_ROOT_POOLNAME] + bkconfigs.ZFS_DATASETS )
    # See ant report if any is not mounted:
    not_mounted_zfss = [
        nmf for nmf
        in full_zfs_list
        if not any ( bkhelpers.find_mounted_fs(zfs_mounteds, nmf, bkhelpers.datasetname_to_path(nmf)) )
        ]

...
class TestAuditFolders(unittest.TestCase):
    ...
    def test_audit_folders_lists_everything(self):
        ...
        self.assertEqual(splotten_stdout, [
            ...
            "./test_fixtures/test_audit_folders/folder2/folder3/mps.zip         >> Scanning inside zipfile ... >>",
            "    > mps/fla/Mountpoints.zip                                          !!! Sensitive name fragment 'mountpoint'.",
            "    > mps/fla/bababakeybobobo                                          !!! Sensitive name fragment 'key'.",
            "    > mps/fla/file.key                                                 !!! Sensitive extension 'key'.",
            "    > mps/fla/file.pem                                                 !!! Sensitive extension 'pem'.",
            "    > mps/fla/file.ppk                                                 !!! Sensitive extension 'ppk'.",
            "    > mps/fla/id_ed25519                                               !!! Sensitive name fragment 'id_ed25519'.",
            "    > mps/fla/id_rsa                                                   !!! Sensitive name fragment 'id_rsa'.",
            "    > mps/flb/backup_colornote_mybake                                  !!! Sensitive name fragment 'colornote'.",
            "    > mps/flb/containes_text                                           OK",
            "    > mps/flb/file.p12                                                 !!! Sensitive extension 'p12'.",
    ...
    def test_audit_folders_omits_oks(self):
        """
        OK files can be omited via the «--noscan ok» flag.
        """
        prog_run = subprocess.run([ './audit_folders.py', '--noscan', 'ok', './test_fixtures/test_audit_folders' ],
            check=True, stdout=subprocess.PIPE)
        splotten_stdout = [ l for l in str(prog_run.stdout, 'utf-8').split('\n') if len(l)>0 ]
        self.assertEqual(splotten_stdout, [
            "./test_fixtures/test_audit_folders/folder1/Mountpoints.zip         !!! Sensitive name fragment 'mountpoint'.",
